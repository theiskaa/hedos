...
private func sdxlRecord() -> ModelRecord {
    ModelRecord(
        name: "sdxl-turbo",
        modality: .image,
        capabilities: [.image],
        source: ModelSource(
            ...
            path: "~/.cache/huggingface/hub/models--stabilityai--sdxl-turbo",
            repo: "stabilityai/sdxl-turbo"),
        runtime: RuntimeRef(
            id: "python:diffusers",
            resolved: .auto,
            tier: .managed),
        params: PipelineFamilyRegistry.sdxlParams,
        execution: .job,
        footprintMB: 7000,
        state: .ready,
...
@Test func diffusersAdapterBidMatrix() {
    let adapter = DiffusersAdapter()
    let record = sdxlRecord()

    let runnableClasses = [
        "StableDiffusionXLPipeline", "StableDiffusionPipeline",
        "KandinskyV22Pipeline", "LatentConsistencyModelPipeline", "FluxPipeline",
    ]
    for pipelineClass in runnableClasses {
        let identified = IdentifiedModel(
            format: .diffusers, modality: .image, capabilities: [.image], execution: .job,
            params: PipelineFamilyRegistry.fluxParams, pipelineClass: pipelineClass)
        let bid = adapter.bid(record, identified)
        #expect(bid?.tier == .managed)
        #expect(bid?.preference == 26)
        #expect(bid?.alternatives == [])
    }

    let classless = IdentifiedModel(
        format: .diffusers, modality: .image, capabilities: [.image], execution: .job)
    #expect(adapter.bid(record, classless) != nil)

    let identifyOnlyVideo = IdentifiedModel(
        format: .diffusers, modality: .video, capabilities: [], execution: .job,
        pipelineClass: "CogVideoXPipeline")
    let identifyOnlyEdit = IdentifiedModel(
        format: .diffusers, modality: .image, capabilities: [], execution: .job,
        pipelineClass: "StableDiffusionXLImg2ImgPipeline")
    let unknownClass = IdentifiedModel(
        format: .diffusers, modality: nil, capabilities: [], execution: .job,
        pipelineClass: "SomePipeline")
    ...
    #expect(adapter.bid(record, identifyOnlyVideo) == nil)
    #expect(adapter.bid(record, identifyOnlyEdit) == nil)
    #expect(adapter.bid(record, unknownClass) == nil)
...
@Test func resolvesHFCacheSDXLTurboToDiffusersManagedWithTurboSchema() async throws {
    let dir = try Fixtures.tempDirectory()
    defer { try? FileManager.default.removeItem(at: dir) }
    ...
    try DiscoveryFixtures.makeHFRepo(
        ...
        DiscoveryFixtures.HFRepo(
            org: "stabilityai", repo: "sdxl-turbo",
            files: [("unet.fp16.safetensors", 8192)],
            modelIndexJSON: DiscoveryFixtures.sdxlModelIndex,
            schedulerConfigJSON: DiscoveryFixtures.turboSchedulerConfig))
    ...
    let record = ModelRecord(
        ...
        source: ModelSource(
            kind: .huggingfaceCache,
            path: hubRoot.appendingPathComponent("models--stabilityai--sdxl-turbo").path,
            repo: "stabilityai/sdxl-turbo",
            ref: "abc123def456"))
    ...
    #expect(resolved.runtime.alternatives == [])
    ...
    #expect(size.values == ["768x768", "1024x1024", "1152x896", "896x1152"])
...
@Test func overridingFluxToDiffusersIsAcceptedAndRoutes() async throws {
    let dir = try Fixtures.tempDirectory()
    defer { try? FileManager.default.removeItem(at: dir) }
    let hubRoot = dir.appendingPathComponent("hub")
    try DiscoveryFixtures.makeHFRepo(
    ...
    try await kernel.overrideRuntime(record.id, to: "python:diffusers")
    let updated = try #require(try await kernel.registry.get(id: record.id))
    #expect(updated.runtime.id == "python:diffusers")
    #expect(updated.runtime.resolved == .user)
    #expect(updated.runtime.tier == .managed)
    #expect(updated.runtime.alternatives.contains("python:mflux"))
    #expect(DiffusersAdapter().canServe(updated, .image))
    #expect(!MfluxAdapter().canServe(updated, .image))
}

@Test func diffusersImageJobLandsPNGArtifactWithProvenance() async throws {
    let dir = try Fixtures.tempDirectory()
    defer { try? FileManager.default.removeItem(at: dir) }
    ...
    #expect(fractions == [1.0 / 3.0, 2.0 / 3.0, 1.0])
    #expect(
        events.contains {
...
@Test func diffusersCancelMidDiffusionUnwindsJobAndNextSubmitReusesSidecar() async throws {
    let dir = try Fixtures.tempDirectory()
    defer { try? FileManager.default.removeItem(at: dir) }
    let spec = imageSidecarSpec()
    let supervisor = SidecarSupervisor()
...
@Test func diffusersAdapterRefusesToStreamImages() async throws {
    let adapter = DiffusersAdapter()
    let stream = adapter.invoke(sdxlRecord(), .image, payload: .null)
