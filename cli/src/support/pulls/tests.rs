...
use kernel::install::pulls::{PullEvent, PullState, START_GRACE_MS};

use crate::support::pulls::testing::{TempDir, job as make_job, moved, status};
...
fn a_waiting_retry_is_the_note_worth_showing() {
    ...
    assert_eq!(note(&status, false, 10_000), "retry in 45s");
}
...
fn a_message_outranks_the_providers_last_line() {
    ...
    assert_eq!(note(&status, false, 10_000), "needs a token");
}
...
fn an_attempt_past_the_first_is_worth_saying_on_its_own() {
    ...
    retried.attempt = 3;
    assert_eq!(note(&retried, false, 10_000), "attempt 3");
    assert_eq!(note(&status(PullState::Running), false, 10_000), "");
}
...
fn a_pull_no_worker_took_up_says_so_rather_than_reading_as_a_queue() {
    ...
    assert_eq!(
        note(
            &status(PullState::Queued),
            job.abandoned(1_000, START_GRACE_MS),
            1_000
        ),
        ""
    );
    assert_eq!(
        note(
            &status(PullState::Queued),
            job.abandoned(99_000, START_GRACE_MS),
            99_000
        ),
        "no worker"
    );
