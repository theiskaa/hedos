//! The FFI backend over the Swift MLX shim: it loads `libhedos_mlx_shim.dylib`,
//! probes availability, and streams generations through the C ABI documented at
//! the top of `shim-mlx/Sources/HedosMlxShim/shim.swift`. Only compiled on
//! macOS; every failure to load degrades to the missing backend rather than
//! erroring, so a binary built without the shim (or run on a Mac that cannot
//! serve it) simply falls through to the Python `mlx-lm` sidecar.
...
use super::backend::{
    MissingMlxSwiftBackend, MlxSwiftBackend, MlxSwiftEvent, MlxSwiftEventStream, MlxSwiftOptions,
};
use super::wire::{done_event, request_json, tool_call_event};
use crate::adapters::{RuntimeError, RuntimeStream};

/// The shim ABI this backend speaks (`hedos_mlx_abi_version`).
...
/// The MLX-Swift backend for this build: the FFI backend when the shim dylib
/// loads and speaks our ABI, else the missing placeholder.
pub fn loaded_mlx_swift_backend() -> Arc<dyn MlxSwiftBackend> {
    match shim() {
        Some(shim) => Arc::new(FfiMlxBackend { shim }),
        None => Arc::new(MissingMlxSwiftBackend),
    }
}
...
/// Where the shim may live, most-specific first: an explicit override, next to
/// the running binary, then the path baked in by the build script (which only
/// holds on the machine that built this binary).
fn candidate_paths() -> Vec<PathBuf> {
    let mut paths = Vec::new();
    if let Some(overridden) = std::env::var_os("HEDOS_MLX_SHIM") {
        paths.push(PathBuf::from(overridden));
    }
    if let Ok(exe) = std::env::current_exe()
        && let Some(dir) = exe.parent()
    {
        paths.push(dir.join("libhedos_mlx_shim.dylib"));
    }
    // Empty when the building toolchain could not build the MLX-Swift shim.
    let baked = env!("HEDOS_MLX_SHIM_BUILT_DYLIB");
    if !baked.is_empty() {
...
fn load(path: &Path) -> Option<Shim> {
    ...
    // SAFETY: the dylib is hedos's own shim; loading runs no initializer beyond
    // Swift/MLX runtime setup, and each symbol is verified to exist before use
    // with the signature the shim's ABI contract documents.
    unsafe {
        let library = Library::new(path).ok()?;
        let version = *library.get::<AbiVersionFn>(b"hedos_mlx_abi_version").ok()?;
        if version() != SUPPORTED_ABI {
            return None;
        }
        let available = *library.get::<AvailabilityFn>(b"hedos_mlx_available").ok()?;
        let stream = *library.get::<StreamFn>(b"hedos_mlx_stream").ok()?;
        let cancel = *library.get::<CancelFn>(b"hedos_mlx_cancel").ok()?;
        Some(Shim {
            available,
            stream,
            cancel,
...
struct CallbackState {
    tx: UnboundedSender<Result<MlxSwiftEvent, RuntimeError>>,
}
...
unsafe extern "C" fn on_event(ctx: *mut c_void, kind: i32, payload: *const c_char) {
    // SAFETY: `ctx` is the `Box<CallbackState>` handed to `hedos_mlx_stream`;
    // the shim invokes this callback under its emit lock and never after the
    // terminal event, so the shared reference cannot alias a free.
    ...
    let terminal = match kind {
        0 => {
            let _ = state.tx.send(Ok(MlxSwiftEvent::Text(text)));
            false
        }
        1 => {
            let _ = state.tx.send(Ok(MlxSwiftEvent::Thinking(text)));
            false
        }
        2 => {
            // A malformed call payload is dropped, keeping the stream alive for
            // the terminal event that still follows.
        ...
        3 => {
            let _ = state.tx.send(Ok(MlxSwiftEvent::Status(text)));
            false
        }
        4 => {
...
struct FfiMlxBackend {
    shim: &'static Shim,
}

impl MlxSwiftBackend for FfiMlxBackend {
    fn is_available(&self) -> bool {
        // SAFETY: a verified no-argument entry point of the loaded shim.
        unsafe { (self.shim.available)() == 1 }
    }
    ...
    fn stream(
        ...
        let Ok(request) = CString::new(request_json(&model_dir, &messages, &tools, &options))
        else {
        ...
        if handle == 0 {
            // Decode failure: the error event already arrived synchronously and
            // freed `state`; the stream will yield it.
            return stream;
        }
        let cancel = self.shim.cancel;
        tokio::spawn(async move {
            // Fires when the consumer drops the stream — including long after a
            // normal finish, where cancelling a finished handle is a no-op.
            tx.closed().await;
